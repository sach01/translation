...
# Morphology Model (Detailed Inflection and Derivation Rules)
class Morphology(models.Model):
    language = models.ForeignKey(Language, on_delete=models.CASCADE, related_name='morphologies')
    verb_conjugation_rules = models.TextField()  # Inflection rules for verbs
    noun_pluralization_rules = models.TextField()  # Pluralization rules for nouns
    derivational_rules = models.TextField()  # Derivational rules for word formation

    def __str__(self):
        return f"Morphology for {self.language.name}"
